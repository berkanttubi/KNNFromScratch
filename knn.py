...
def calculate_distances(train_data, test_instance, distance_metric):
    ...
    if (distance_metric=="L1"):
        for x,y,z,t in train_data:
            distances.append(abs(test_instance[0]-x) + abs(test_instance[1]-y)+abs(test_instance[2]-z)+abs(test_instance[3]-t))
            
    elif (distance_metric=="L2"):
        for x,y,z,t in train_data:
            temp = (test_instance[0]-x)**2 + (test_instance[1]-y)**2 + (test_instance[2]-z)**2 + (test_instance[3]-t)**2
...
def majority_voting(distances, labels, k):
    ...
    for i in range(k):
        if (labels[np.argmin(distances)] == 1):
            one_labelled+=1
        elif (labels[np.argmin(distances)] == 0):
            zero_labelled+=1
        else:
            two_labelled+=1
    ...
    if zero_labelled > one_labelled and zero_labelled > two_labelled :
        return 0
    elif zero_labelled < one_labelled and two_labelled < one_labelled :
        return 1
    elif two_labelled > one_labelled and two_labelled > zero_labelled :
        return 2
    else:
        return 0
